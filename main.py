...
while playing:
    ...
    for event in pygame.event.get():
        ...
        if event.type == CREATE_BONUS:
            more_bonus.append(create_bonus())
    ...
    for bonus in more_bonus:
        bonus[1] =bonus[1].move(bonus[2])
        main_display.blit(bonus[0], bonus[1])
        
        if player_rect.colliderect(bonus[1]):
            score+=1
            more_bonus.pop(more_bonus.index(bonus))
            
    main_display.blit(FONT.render(str(score),True,BLACK_COLOR), (WIDTH-50, 20))
